...
class SpringInterceptorConfig(
    ...
    @Bean
    fun service(): TenantInterceptorService = InMemoryTenantInterceptor(serviceKey, sandboxKey, client())

    @Bean
    fun filter(): TenantInterceptorFilter = TenantInterceptorFilter(tenantHeader, service())
}
